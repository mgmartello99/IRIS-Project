...
// https://svn.freebsd.org/base/head/lib/msun/src/s_log1pf.c
...
EXTERN_C LslFloatC_t bsd_log1pf(float x)
{
	...
	{			/* 1+x < sqrt(2)+  */
		...
		{		/* x <= -1.0 */
			if (x == (float)-1.0)
			{
				RETURN_FLOAT(-two25 / vzero); /* log1p(-1)=+inf */
			}
			else
			{
				RETURN_FLOAT((x - x) / (x - x));	/* log1p(x<-1)=NaN */
			}
		}
		if (ax < 0x38000000)
		{			/* |x| < 2**-15 */
	...
	{	/* |f| < 2**-20 */
		...
		{
			RETURN_FLOAT(k * ln2_hi - ((R - (k*ln2_lo + c)) - f));
		}
	}
	s = f / ((float)2.0 + f);
	z = s * s;
	...
	{
		RETURN_FLOAT(k * ln2_hi - ((hfsq - (s*(hfsq + R) + (k*ln2_lo + c))) - f));
	}
	EPILOGUE_FLOAT();
}

// [DEVSW-1877, 8D-5889] grumat: <math.h> Functions are missing 
EXTERN_C float log1pf(float x) { return L2F_(bsd_log1pf(x)); }
