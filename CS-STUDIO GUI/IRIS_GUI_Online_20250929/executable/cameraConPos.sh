...
# Forwarding CCTV camera IP addresses, the connection of this tunnel does not close if the cctv streams are read by mpv
ssh -L 8202:10.5.0.202:555 -L 8203:10.5.0.203:554 -L 8204:10.5.0.204:554 -L 8205:10.5.0.205:554 -L 8206:10.5.0.206:554 -Y isolpharm_iris@10.23.14.8
...
# Bottom-right window
mpv --video-crop=480x360+80+0 --video-rotate=0 --geometry=${CamBotW}x${CamBotH}+${CamBotRPosX}+${CamBotPosY} --title="CCTV4 10.5.0.205 - Detection position" --screen=${screenid} rtsp://isolpharm:isolpharm@localhost:8205/hsub&
