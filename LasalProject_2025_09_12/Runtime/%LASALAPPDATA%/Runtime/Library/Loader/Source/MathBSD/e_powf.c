...
// https://svn.freebsd.org/base/head/lib/msun/src/e_powf.c
...
EXTERN_C LslFloatC_t bsd_powf(float x, float y)
{
	...
	{	/* y is +-inf */
		if (ix == 0x3f800000)
		{
			RETURN_FLOAT(one);	/* (-1)**+-inf is NaN */
		}
		else if (ix > 0x3f800000)/* (|x|>1)**+-inf = inf,0 */
		{
			RETURN_FLOAT((hy >= 0) ? y : zero);
		}
		else			/* (|x|<1)**-,+inf = inf,0 */
		{
			RETURN_FLOAT((hy < 0) ? -y : zero);
	...
	if (hy == 0x40000000)
	{
		RETURN_FLOAT(x * x); /* y is  2 */
	}
	if (hy == 0x3f000000)
	{	/* y is  0.5 */
		if (hx >= 0)	/* x >= +0 */
		{
			RETURN_L2F (bsd_sqrtf(x));
		}
	}
...
/* over/underflow if x is not close to one */
		if (ix < 0x3f7ffff8)
		{
			RETURN_FLOAT((hy < 0) ? sn * huge*huge : sn * tiny*tiny);
		}
		if (ix > 0x3f800007)
		{
			RETURN_FLOAT((hy > 0) ? sn * huge*huge : sn * tiny*tiny);
	...
	{
		RETURN_FLOAT(sn * huge*huge);			/* overflow */
	}
	...
	{
		RETURN_FLOAT(sn * tiny*tiny);			/* underflow */
	}
	...
	GET_FLOAT_WORD(j, z);
	j += (n << 23);
	if ((j >> 23) <= 0) z = lsl_scalbnf(z, n);	/* subnormal output */
