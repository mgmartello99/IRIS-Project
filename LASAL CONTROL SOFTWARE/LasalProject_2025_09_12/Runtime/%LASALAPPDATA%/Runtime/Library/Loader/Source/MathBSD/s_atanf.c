 ...
 */
 ...
 // https://svn.freebsd.org/base/head/lib/msun/src/s_atanf.c
...
EXTERN_C LslFloatC_t bsd_atanf(float x)
{
	...
	{	/* if |x| >= 2**26 */
		...
		if (hx > 0)
		{
			RETURN_FLOAT (atanhi[3] + *(volatile float *)&atanlo[3]);
		}
		else
		{
			RETURN_FLOAT(-atanhi[3] - *(volatile float *)&atanlo[3]);
   ...
   /* end of argument reduction */
	...
	if (id < 0)
	{
		RETURN_FLOAT (x - x * (s1 + s2));
	}
	else
	{
		z = atanhi[id] - ((x*(s1 + s2) - atanlo[id]) - x);
		RETURN_FLOAT ((hx < 0) ? -z : z);
	}
	EPILOGUE_FLOAT();
}
