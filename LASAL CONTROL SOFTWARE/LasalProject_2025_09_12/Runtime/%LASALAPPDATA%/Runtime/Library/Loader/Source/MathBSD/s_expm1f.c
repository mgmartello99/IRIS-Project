...
// https://svn.freebsd.org/base/head/lib/msun/src/s_expm1f.c


#include "MathBSD.h"
#include "math_private.h"

DO_GCC_PRAGMA(GCC diagnostic ignored "-Wuninitialized")
...
static volatile float huge = 1.0e+30f;
...
EXTERN_C LslFloatC_t bsd_expm1f(float x)
{
	...
	{			/* if |x|>=27*ln2 */
		...
		{		/* if |x|>=88.721... */
			...
			if (hx == 0x7f800000)
			{
				RETURN_FLOAT((xsb == 0) ? x : -1.0f);/* exp(+-inf)={inf,-1} */
			}
			if (x > o_threshold)
			{
		...
		{ /* x < -27*ln2, return -1.0 with inexact */
			...
			{
				RETURN_FLOAT(tiny - one);	/* return -1 */
			}
		}
	}
	...
	else
	{
		...
		if (k == -1)
		{
			RETURN_FLOAT((float)0.5*(x - e) - (float)0.5);
		}
		if (k == 1)
		{
			if (x < (float)-0.25)
			{
				RETURN_FLOAT(-(float)2.0*(e - (x + (float)0.5)));
			}
			else
			{
				RETURN_FLOAT(one + (float)2.0*(x - e));
			}
		}
		if (k <= -2 || k > 56)
...
EXTERN_C float expm1f(float x) { return L2F_(bsd_expm1f(x)); }
