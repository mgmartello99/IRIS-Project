...
// https://svn.freebsd.org/base/head/lib/msun/src/e_hypot.c
...
EXTERN_C double bsd_hypot(double x, double y)
{
	...
	{	/* a>2**500 */
		...
		{	/* Inf or NaN */
			...
			w = bsd_fabs(x + 0.0) - bsd_fabs(y + 0.0);
			GET_LOW_WORD(low, a);
			if (((ha & 0xfffff) | low) == 0)
				w = a;
			GET_LOW_WORD(low, b);
	...
	else
	{
		...
		t2 = a - t1;
		w = bsd_sqrt(t1*y1 - (w*(-w) - (t1*y2 + t2 * b)));
	}
	if (k != 0)
	{
		u_int32_t high;
...
EXTERN_C double hypot(double x, double y) ALIAS(bsd_hypot);
